...
void TCPApp_c::TCPServer(void *arg)
{
  ...
  struct tcp_sock *tsk = TCPSock->alloc_tcp_sock();

  struct sock_addr addr;
  addr.IP = 0;
  addr.port = port;
  if (TCPSock->tcp_sock_bind(tsk, &addr) < 0) {
    printf("tcp_sock bind to port 0x%02x failed\n", port);
    exit(1);
  }

  if (TCPSock->tcp_sock_listen(tsk, 3) < 0) {
  ...
  struct tcp_sock *csk = TCPSock->tcp_sock_accept(tsk);

  printf("accept a connection.\n");

  sleep(5);

  TCPSock->tcp_sock_close(csk);
...
void TCPApp_c::TCPClient(void *arg)
{
  ...
  if (TCPSock->tcp_sock_connect(tsk, skaddr) < 0) {
